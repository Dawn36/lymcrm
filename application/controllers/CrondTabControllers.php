...
class CrondTabControllers extends CI_Controller
{
	...
	public function SetEmail()
	{
		...
		$data = $this->CRONDTAB->TenancyFromTenant($whereThirtyDays);
		if (!empty($data)) {
			$this->SendEmail($data);
		}

		$whereSixtyDays = "DATE(`end_date`) = '$sixtyDays'";
		$data = $this->CRONDTAB->TenancyFromTenant($whereSixtyDays);
		if (!empty($data)) {
			$this->SendEmail($data);
		}

		$whereNinetyDays = "DATE(`end_date`) = '$ninetyDays'";
		$data = $this->CRONDTAB->TenancyFromTenant($whereNinetyDays);
		if (!empty($data)) {
			$this->SendEmail($data);
		}
	}
