...
class Infection extends CI_Controller
{
	...
	public function formDataSubmit()
	{
		...
		$arrInfo['mr_no'] = strtolower($post['mr_num']);
		$arrInfo['pateint_name'] = strtolower($post['names']);
		$arrInfo['ward_no'] = $post['ward'];
		$arrInfo['bed_no'] = $post['bed'];
		$arrInfo['primary_diagnosis'] = strtolower($post['primary_diagnosis']);
		$arrInfo['final_diagnosis'] = strtolower($post['final_diagnosis']);
		$discharge_status = strtolower($post['discharge_status']);
		$arrInfo['admission_date'] = date("Y-m-d", strtotime($post['date_of_admission']));
		if ($discharge_status == "not applicable" || $discharge_status == "") {
			$arrInfo['discharge_date'] = "";
		} else {
			$arrInfo['discharge_date'] = date("Y-m-d", strtotime($post['date_of_discharge']));
		}

		$arrInfo['discharge_condition'] = strtolower($post['condition_on_discharge']);
		$arrInfo['fever'] = strtolower($post['fever']);
		$arrInfo['temp'] = $post['temperature_'];
		$arrInfo['tachy'] = strtolower($post['tachycardia']);
		$arrInfo['pulse'] = $post['pulse'];


		$arrInfo['hypo'] = strtolower($post['hypotension']);
		$arrInfo['bp'] = $post['bp'];
		$arrInfo['chills'] = strtolower($post['chills']);
		$arrInfo['wbc'] = $post['wbc'];
		...
		$arrInfo['form_creation'] = date("Y-m-d h:i:s");

		if ($arrInfo != '') {
			// log_message('debug',  "dsadasd");
			//$this->load->model('Model_infection');
			$insertId = $this->MI->InfectionForm($arrInfo);
		}

		//if ($check) {
			
			if ($this->input->post('intravenous_cannula') == 'yes') {
				$arrInfoPhlebitis['form_id'] = $insertId;
				...
				$arrInfoPhlebitis['mr_no'] = $this->input->post('mr_num');
				$arrInfoPhlebitis['device_creation'] = date("Y-m-d");
				$arrInfoPhlebitis['created_by'] = $this->session->userdata('user_id');
				$arrInfoPhlebitis['created_at'] = Date('Y-m-d');
				...
				if ($this->input->post('phlebitis_culture_report') == 'yes') {
					...
					for ($i = 0; $i < count($arrPhlebitisCultureReport['phlebitis_antibiotic']); $i++) {
						...
						$infoAdd['created_by'] = $this->session->userdata('user_id');
						// $infoAdd['created_at'] = Date('Y-m-d');
			...
			if ($this->input->post('culture_report') == 'yes') {
				//$arrPhlebitisCultureReport['culture_report']=$this->input->post('culture_report');

				$arrPhlebitisCultureReport['culture_report_date'] = $this->input->post('culture_report_date');
				$arrPhlebitisCultureReport['culture_report_source'] = $this->input->post('culture_report_source');
				$arrPhlebitisCultureReport['culture_report_result'] = $this->input->post('culture_report_result');
				$arrPhlebitisCultureReport['culture_report_organism'] = $this->input->post('culture_report_organism');
				$arrPhlebitisCultureReport['culture_report_antibiotic'] = $this->input->post('culture_report_antibiotic');
				$arrPhlebitisCultureReport['culture_report_mdr_name'] = $this->input->post('culture_report_mdr_name');

				for ($i = 0; $i < count($arrPhlebitisCultureReport['culture_report_antibiotic']); $i++) {
					for ($j = 0; $j <= $i; $j++) {
						if ($arrPhlebitisCultureReport['culture_report_antibiotic'] != '') {
							$antibiotic = implode(",", $arrPhlebitisCultureReport['culture_report_antibiotic'][$j]);
						} else {
					...
					$infoAdd['form_id'] = $insertId;
					$infoAdd['device_id'] = 0;
					$infoAdd['culture_insertion_date'] = date("Y-m-d", strtotime($arrPhlebitisCultureReport['culture_report_date'][$i]));
					$infoAdd['culture_source'] = $arrPhlebitisCultureReport['culture_report_source'][$i];
					$infoAdd['culture_result'] = $arrPhlebitisCultureReport['culture_report_result'][$i];
					$infoAdd['culture_organism'] = $arrPhlebitisCultureReport['culture_report_organism'][$i];
					$infoAdd['culture_antibio'] = $antibiotic;
					$infoAdd['MDR'] = $arrPhlebitisCultureReport['culture_report_mdr_name'][$i];
					$infoAdd['culture_creation'] = date("Y-m-d");
					$infoAdd['created_by'] = $this->session->userdata('user_id');
						// $infoAdd['created_at'] = Date('Y-m-d');
					$this->Model_infection->InfectionFormCulture($infoAdd);
			...
			if ($this->input->post('urinary_catheter') == 'yes') {
				$arrInfoUrinary['form_id'] = $insertId;
				...
				$arrInfoUrinary['created_by'] = $this->session->userdata('user_id');
				$arrInfoUrinary['created_at'] = Date('Y-m-d');
				$this->load->model('Model_infection');
			...
			if ($this->input->post('central_line') == 'yes') {
				$arrInfoCentral['form_id'] = $insertId;
				...
				$arrInfoCentral['created_by'] = $this->session->userdata('user_id');
				$arrInfoCentral['created_at'] = Date('Y-m-d');
				$this->load->model('Model_infection');
			...
			if ($this->input->post('hospital_acquired_pneumonia') == 'yes') {
				$arrInfoHospital['form_id'] = $insertId;
				...
				$arrInfoHospital['created_by'] = $this->session->userdata('user_id');
				$arrInfoHospital['created_at'] = Date('Y-m-d');
				$this->load->model('Model_infection');
			...
			if ($this->input->post('ventilator_associated_pneumonia') == 'yes') {
				$arrVentilatorCultureReport['form_id'] = $insertId;
				...
				$arrVentilatorCultureReport['created_by'] = $this->session->userdata('user_id');
				$arrVentilatorCultureReport['created_at'] = Date('Y-m-d');
				$this->load->model('Model_infection');
			...
			if ($this->input->post('surgical_site_infection') == 'yes') {
				$arrInfoSurgical['form_id'] = $insertId;
				...
				$arrInfoSurgical['created_by'] = $this->session->userdata('user_id');
				$arrInfoSurgical['created_at'] = Date('Y-m-d');
				$this->load->model('Model_infection');
