...
class Infection extends CI_Controller {
	...
	public function formDataSubmit()
	{
		 ...
		 $arrInfo['mr_no']=$this->input->post('mr_num');
		 $arrInfo['pateint_name']=$this->input->post('names');
		 $arrInfo['ward_no']=$this->input->post('ward');
		 $arrInfo['bed_no']=$this->input->post('bed');
		 $arrInfo['primary_diagnosis']=$this->input->post('primary_diagnosis');
		 $arrInfo['final_diagnosis']=$this->input->post('final_diagnosis');
		 ...
		 $arrInfo['discharge_condition']=$this->input->post('condition_on_discharge');
		 $arrInfo['fever']=$this->input->post('fever');
		 $arrInfo['temp']=$this->input->post('temperature_');
		 $arrInfo['tachy']=$this->input->post('tachycardia');
		 $arrInfo['pulse']=$this->input->post('pulse');
		 $arrInfo['hypo']=$this->input->post('hypotension');
		 $arrInfo['bp']=$this->input->post('bp');
		 $arrInfo['chills']=$this->input->post('chills');
		 $arrInfo['wbc']=$this->input->post('wbc');
		 $arrInfo['neutro']=$this->input->post('ln');
		 $arrInfo['lymp']=$this->input->post('lymphocytes');
		 $arrInfo['worse_cxr']=$this->input->post('worsening_cxr');
		 $arrInfo['worse_tracheal_secret']=$this->input->post('worsening_tracheal_secretions');
		...
		if ($this->input->post('intravenous_cannula') == 'yes')
		{
			...
			$arrInfoPhlebitis['site']=$this->input->post('phlebitis_sife');
		 	$arrInfoPhlebitis['phlebitis']=$this->input->post('phlebitis_phlebitis');
		 	$arrInfoPhlebitis['check']=$this->input->post('phlebitis_options');
			...
			//$arrPhlebitisCultureReport['phlebitis_culture_report']=$this->input->post('phlebitis_culture_report');
			 ...
			 for ($i=0; $i <count($arrPhlebitisCultureReport['phlebitis_date']); $i++) 
			 { 
			 	for($j=0 ; $j<=$i; $j++)
			 	{
			...
			if($this->input->post('urinary_catheter') == 'yes')
			{
				...
				// $arrUrinaryCultureReport['urinary_culture_report']=$this->input->post('urinary_culture_report');
		 		...
		 		for ($i=0; $i <count($arrUrinaryCultureReport['urinary_date']); $i++) 
				 { 
				 	for($j=0 ; $j<=$i; $j++)
			 	{
		 ...
		 $arrCentralCultureReport['central_mdr_name']=$this->input->post('central_mdr_name');	
			...
			$arrInfoHospital['deivce_insertion_date']=date("Y-m-d", strtotime($this->input->post('hospital_date_of_insertion')));
			...
			$arrVentilatorCultureReport['deivce_insertion_date']=date("Y-m-d", strtotime($this->input->post('ventilator_date_of_insertion')));
